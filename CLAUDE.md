...
**Phase 2 — Routing, landing page & All Projects page — not started.** Moves `(estimator)` under `/project/[projectId]`, adds `/` (landing: Create New Project / Explore Current Projects) and `/projects` (All Projects table + Name/Client filters + Edit/Delete + Master Defaults button), rewires `EstimateContext` off `localStorage` onto `saveProjectDraft`/a new `flushSave()`, updates the Sidebar.
...
Continue sub-project A: write and execute Phase 2's implementation plan (routing/landing/All-Projects), then Phase 3 (per-project Admin).
