...
- **Database:** Postgres via Prisma ORM (Prisma Postgres in production; a dedicated Neon dev database is used for local dev/test — see below; local Docker Postgres was retired 2026-08-14, see below)
- **Testing:** Vitest
...
**As of 2026-08-14, local dev/test uses a dedicated Neon Postgres database, not Docker.** The prior local Docker Postgres container was retired after a host disk-space exhaustion incident took down Docker Desktop's WSL2 backend mid-project; rather than restore Docker, dev/test was pointed at a separate free-tier Neon project (never production — production is Prisma Postgres, a different provider/instance).

`.env` (gitignored) needs both a pooled and a direct connection string from that Neon project:
```
DATABASE_URL="postgresql://...-pooler.<region>.aws.neon.tech/neondb?sslmode=require&pgbouncer=true&connection_limit=10"
DIRECT_URL="postgresql://...<region>.aws.neon.tech/neondb?sslmode=require"
```
`connection_limit` must be >1 — several code paths (e.g. `createProject()`) issue many queries concurrently via `Promise.all`, and Neon's pooled endpoint will otherwise queue and time out (Prisma error P2024) waiting for a single connection slot.

Run `npx prisma migrate deploy` then `npm run seed` once against a fresh Neon database before first use.

**Known tradeoff:** remote network latency makes the test suite much slower than local Postgres was — `vitest.config.ts`'s `testTimeout`/`hookTimeout` are raised to 60s to accommodate this (the default 5s, then 30s, both proved too short for `createProject()`'s 9-table clone transaction over a real network round-trip). A full `npx vitest run` also intermittently hits a native-module SIGSEGV after ~15-20 minutes of continuous execution (a Prisma/Node native-binding stability issue under sustained long-lived remote connections, not a code or test defect — every test that runs before the crash passes cleanly). If a full run crashes, split it into 2-3 batches by file list rather than fighting one long-lived process; each batch completing green is equivalent evidence to one clean full run.

`.env.example` documents the shape; copy to `.env` and adjust before running `npx prisma migrate deploy`/`migrate dev` or `npm run seed`.
...
## Status: Multi-Project Support — COMPLETE (merged to main)
...
**Phase 3 — Per-project Admin — COMPLETE.** Plan: `docs/superpowers/plans/2026-08-12-per-project-admin.md`, executed via `superpowers:subagent-driven-development` (8 tasks, each independently implemented and reviewed, plus a final whole-branch review on opus).
- Added a new unauthenticated `/project/[projectId]/admin/...` area mirroring the 5 existing master admin sections (Materials, Labor Tasks, Rates, Pass Throughs, Defaults), targeting the `Project*` tables from Phase 1 instead of the master ones. Resolves Phase 2's carried-over security note: no auth on per-project admin is confirmed-deliberate (the master `/admin` stays password-gated; per-project admin was always meant to be open, per the user's original request for this sub-project).
- The estimator was isolated under a nested `(estimator)` route group first (Task 1) specifically so the new `admin/` sibling wouldn't inherit its `Sidebar`/`AppShell` — confirmed live, no estimator chrome bleeds into any admin page.
- Every per-project mutation follows the same safety pattern established here: `updateMany`/`deleteMany` scoped by `{ id, projectId }` together with a `result.count === 0` check, catching a mismatched `(projectId, id)` pair instead of silently no-op'ing or (worse) matching the wrong project's row. All 9 applicable update/delete pairs across the 5 sections have an adversarial test proving this — a systemic test-coverage gap (early tasks only proved two *different* rows don't collide, not the actual mismatched-pair case) was caught mid-branch and retrofitted everywhere, then required from-the-start for later tasks.
- Shared admin validation helpers (`ActionResult`/`ValidationErr`/`parseNonNegative`/`parsePercent`/`parseNonNegativeInt`/`parseLabeledPercent`) extracted to `src/lib/admin/validation.ts`, used by both the master and per-project admin action files.
- **Infrastructure change mid-branch:** the team's local Docker Postgres was permanently retired (a host disk-space exhaustion incident took down Docker Desktop's WSL2 backend) and replaced with a dedicated Neon dev database — see "Local dev database" above for the full detail, including the resulting Vitest timeout/batching adjustments.
- **Two real Critical/Important bugs were found empirically during live-browser verification, neither caught by `tsc`/`vitest`/`npm run build`:**
  1. The Rates and Defaults per-project admin pages passed a Server Action wrapped in a new inline closure (`onSave={(values) => updateProjectX(projectId, values)}`) as a prop from a Server Component to a `'use client'` form — invalid Next.js RSC serialization, 500'ing both pages at runtime. Neither route is ever statically rendered (`force-dynamic`), so the build never caught it. Fixed with `.bind(null, projectId)`, Next's supported pattern for passing a partially-applied Server Action as a prop. **Same lesson as Phase 2's `normalizeDraft` regression: a green build and passing tests are not sufficient evidence for any change crossing the Server/Client boundary — a live render check is required.**
  2. The per-project Defaults admin was a functional no-op for any project with a saved draft: `normalizeDraft()` merges the draft's own copy of markups/tax/contingency OVER fresh `ProjectEstimateDefaults` values, so editing Tax Rate or Corporate Markup % (which have no other editable path) silently had zero effect after the first autosave. This was a genuine product-behavior question, not a mechanical bug, so three fix options were presented to the user rather than picked unilaterally; the user chose write-through. `updateProjectEstimateDefaults` now also merges the 6 edited values into `Project.draftJson` (same transaction, when a draft already exists), preserving every other draft field untouched.
  3. (Related, mechanical) `admin/layout.tsx` had no project-existence guard, unlike the estimator's own layout — an invalid `projectId` produced an inconsistent mix of hollow 200s and hard 500s across the 5 sections. Fixed with the same `notFound()` pattern the estimator layout already used.
- Verified: 186/186 tests passing (36 files, run in 2-3 batches per the Neon native-module-crash note above), `tsc --noEmit` clean, `npm run build` succeeds (22 routes).
- Deferred (Minor, non-blocking): business-rule validators (~450 lines) duplicated verbatim between master and per-project action files, one level past what the shared `validation.ts` extracted — highest-value remaining extraction if this area sees more churn; per-project admin's nav has no active-tab indicator (deliberate, avoids a `'use client'` layout — see the code comment in `admin/layout.tsx`); `LaborProjectionSettingsForm`/`EstimateDefaultsForm` live under the master admin's route group and are imported by the unauthenticated per-project one — works fine, but belongs in `src/components/admin/` if the master admin area is ever reorganized; `BAT Materials` is now structurally unreachable in every project (zero seeded rows + the admin table hides empty categories) — already an accepted scope boundary from sub-project C, just now enforced by UI accident rather than decision; the `updateProjectEstimateDefaults` write-through's `draftJson` read sits outside its transaction, so a concurrent autosave in another tab within that single round-trip could theoretically be dropped — same class as Phase 2's already-accepted autosave races.

## What to do next

All three sub-projects from the user's original request (B: editable Mark-Up %, C: Materials Excel export, A: multi-project support) are complete and merged to `main`. No specific next task is queued — check with the user for what's next.

Minor items noted for later (non-blocking, carried over from earlier phases): the Materials `percentOfTotal` display field's denominator should be reconciled against the workbook's display column; the crew-size technician-count input should be constrained to 1–20 in the UI; consider `next/font` instead of the current Google Fonts `@import` in `globals.css`; PDF generation (`BlobProvider`) regenerates on every summary-page keystroke instead of gating behind the Export click; `Number(e.target.value)` produces `NaN` on empty/partial numeric input across several pages; see Phase 3's deferred-minors list above for the newest additions.
